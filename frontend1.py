...
def search_command():
    clear_list()
    books=backend1.search(title=title_text.get(),author=author_text.get(),year=year_text.get(),isbn=isbn_text.get())
    fill_list(books)
...
def add_command():
    backend1.insert(title=title_text.get(),author=author_text.get(),year=year_text.get(),isbn=isbn_text.get())
    view_command()
...
def update_command():
    backend1.update(selected_book[0],title=title_text.get(),author=author_text.get(),year=year_text.get(),isbn=isbn_text.get())
    view_command()
...
def delete_command():
    backend1.delete(selected_book[0])
    view_command()
